...
function parseOneResultTable(tableHtml) {
  ...
  for (const cells of rawRows) {
    ...
    if (hasPrize1Header && cells[prize1Index]) {
      ...
      if (numberCell) parserMode = 'class';
    }

    if (!numberCell) {
      // IMPORTANT: the live /history/result/{code}/kosong endpoint can return
      // only <tbody>/<tr>/<td> rows without the table header. A 6-column row
      // is typically: Periode | Hari | Tanggal | Nomor | Nomor 2 | Nomor 3.
      // Taking the last cell here would publish Prize 3. Instead, locate the
      // date cell and take the immediate next column, which is Prize 1.
      const inferredDateIndex = cells.findIndex((c, index) =>
        index !== periodIndex && /\d{4}-\d{2}-\d{2}/.test(String(c.text || ''))
      );
      if (inferredDateIndex >= 0 && cells[inferredDateIndex + 1]) {
        numberCell = cells[inferredDateIndex + 1];
        parserMode = 'after-date-prize1';
      } else if (cells.length === 3) {
        // True legacy three-column layout: Periode | Tanggal | Nomor.
        numberCell = cells[2];
        parserMode = 'legacy-3col';
      } else {
        // Never guess on a wider headerless row. A wrong published result is
        // worse than waiting for a parseable source response.
        continue;
      }
    }
    ...
    rows.push({
      ...
      prize1: numberText,
      rawDate: dateRaw,
      _parser: parserMode || (hasRecognizedHeader ? 'header' : 'unknown')
    });
  }
...
async function scanMarket(market, settings, state, { force = false, sourceCache = null } = {}) {
  ...
  if (!force && prev.period && cmp <= 0) {
    // Keep the admin monitor synchronized with the exact date/time text currently
    // shown by /history/number, even when the period itself has not changed.
    state.markets[market.slug] = {
      ...
      sourceDateTime: latest.rawDate || [latest.date, latest.resultTime].filter(Boolean).join(' '),
